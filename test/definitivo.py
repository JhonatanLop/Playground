BOARD_X = [str(i) for i in range(1, 16)]
BOARD_Y = [chr(i) for i in range(65, 81)]
INVALID_LETTERS = ['K']
...
TORPEDO_TOTAL = 25
SHIP_RULES = {1: [4, 5], 2: [5, 2], 3: [1, 10], 4: [2, 5] }
...
def escrever_erro(message):
    escrever_result(message)

def ler_posicao(pos_dir):
    x_number = pos_dir[1:]
    y_letter = pos_dir[0]
...
def add_navio_tab(board, code, value_string):
    code = int(code)
...
def ler_jogador(file_name):
    ...
    for line in data:
        ...
        if code == 'T':
            add_torpedo(torpedo_list, value_string)
            continue
        
        add_navio_tab(board, code, value_string)
        verif_sobreposicao_nav(board)
...
if __name__ == '__main__':
    ...
    try:
        err_scope = 'J1'
        [board_1, torpedo_list_1] = ler_jogador('jogador1')
        
        err_scope = 'J2'
        [board_2, torpedo_list_2] = ler_jogador('jogador2')
        
        bombardeio(board_1, torpedo_list_2)
        bombardeio(board_2, torpedo_list_1)

        score_1 = cont_ponto(board_2)
        score_2 = cont_ponto(board_1)

        if score_1[0] > score_2[0]:
            result_string = gerar_resultado(1, score_1)
            escrever_result(result_string)

        if score_1[0] < score_2[0]:
            result_string = gerar_resultado(2, score_2)
            escrever_result(result_string)

        if score_1[0] == score_2[0]:
            result_string = gerar_resultado(1, score_1)
            result_string += f'\n{gerar_resultado(2, score_2)}'
            escrever_result(result_string)

    except Exception as err:
        escrever_erro(f'{err_scope} {str(err)}')